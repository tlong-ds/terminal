...
extension Ghostty {
    ...
    class App: ObservableObject {
        ...
        init(configPath: String? = nil) {
            print("[bunnyshell] Ghostty.App init start")
            self.configPath = configPath
            if !App.ghosttyInitialized {
                _ = ghostty_init(UInt(CommandLine.argc), CommandLine.unsafeArgv)
                App.ghosttyInitialized = true
            }
            // Initialize the global configuration.
            self.config = Config(at: configPath)
            if self.config.config == nil {
                readiness = .error
                print("[bunnyshell] Ghostty.App init failed: config nil errors=\(self.config.errors)")
                return
            }
            ...
            guard let app = ghostty_app_new(&runtime_cfg, config.config) else {
                ...
                readiness = .error
                print("[bunnyshell] Ghostty.App init failed: ghostty_app_new returned nil errors=\(self.config.errors)")
                return
            }
            self.app = app
            print("[bunnyshell] Ghostty.App init ok appPtr=\(String(describing: self.app))")
            ...
            // Kick an initial tick so the first surface can initialize promptly
            // even if we haven't received a wakeup yet.
