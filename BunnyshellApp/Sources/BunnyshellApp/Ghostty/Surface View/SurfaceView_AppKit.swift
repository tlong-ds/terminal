...
extension Ghostty {
    ...
    class SurfaceView: OSSurfaceView, Codable, Identifiable {
        ...
        init(_ app: ghostty_app_t, baseConfig: SurfaceConfiguration? = nil, uuid: UUID? = nil) {
            ...
            print("[bunnyshell] SurfaceView init uuid=\(uuid?.uuidString ?? "nil")")
            ...
            guard let surface = surface else {
                ...
                print("[bunnyshell] ghostty_surface_new failed uuid=\(self.id.uuidString)")
                return
            }
            self.surfaceModel = Ghostty.Surface(cSurface: surface)

            // Debug visibility into whether the child process actually started.
            // If the shell fails to launch, Ghostty will often still return a surface.
            let pid = ghostty_surface_foreground_pid(surface)
            let exited = ghostty_surface_process_exited(surface)
            let tty = Ghostty.AllocatedString(ghostty_surface_tty_name(surface)).string
            print("[bunnyshell] surface created id=\(self.id.uuidString) pid=\(pid) exited=\(exited) tty=\(tty)")

            // Ensure the core receives a non-zero initial size immediately.
